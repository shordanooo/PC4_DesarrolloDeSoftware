...
async def lifespan(app: FastAPI):
    ...
    # Semilla de datos automática en caso de base de datos vacía
    db = db_manager.conn
    cursor = db.cursor()
    cursor.execute("SELECT COUNT(*) as cnt FROM caretakers")
    res = cursor.fetchone()
    if res and res.get("cnt", 0) == 0:
        print("Poblando base de datos automáticamente con datos semilla...")
        try:
